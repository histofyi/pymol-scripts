...
output_path = '../static/images/structures/cleft/pockets'




def process_pocket_images(pdb_code):
    print (pdb_code)
    for size in ['full','medium','small']:
        try:
            pockets_image = f'tmp/pockets/png/{pdb_code}_pockets_{size}.png'
            pocket_labels = f'tmp/pockets/labels/pocket_labels_{size}.png'
...
def main():
    ...
    for pdb_code in itemset[0]['members']:
        errors = process_pocket_images(pdb_code)
